...
class Command(BaseCommand):
    help = "Comment 데이터를 fixture로 덤프합니다 (UTF-8 인코딩 보장)"

    def add_arguments(self, parser):
        parser.add_argument(
            "--output",
            type=str,
            help="출력 파일 경로 (예: fixtures/comments.json)",
        )
        parser.add_argument(
            "--timestamp",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if options["output"]:
            ...
            filename = f'comments_fixture{"_" + timestamp if timestamp else ""}.json'
        ...
        # Comment 데이터 가져오기
        self.stdout.write("Comment 데이터 덤프 중...")
